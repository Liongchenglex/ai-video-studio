...
### 22. v4 Phase 3 (batch Generate all) — final-review triage roll-up

**Status:** OPEN (none release-blocking; final whole-branch review 2026-07-07
verdict MERGE READY). Recorded from per-task review minors + final triage:

- Poll merge dispatches one patchShot/patchEntity per row unconditionally each
  5s tick during a batch → editor-context re-render churn; add shallow-equal
  skip (belongs with a broader memoization pass).
- batchRemaining label is a project-wide approximation: shows "0 left" during
  a retry-of-failed until the row flips generating; counts pending shots with
  blank imagePrompt that targeting excludes; any single per-item generation
  flips the TopBar into "Generating… N left". Self-correcting; consider a
  run-scoped count if a batch entity ever exists.
- Dialog: benign dispatch outcomes (409 from another tab, nothing-to-do)
  render as generic "Something went wrong. Close and try again."; error copy
  suggests closing though inline retry works; state reset via effect not
  key-remount (theoretical one-render stale flash on reopen).
- Waves don't re-check projects.deletedAt mid-batch — a project soft-deleted
  mid-run keeps spending until the batch drains (one-line join addition).
- Step-time skip guard trades crash-retry auto-recovery for guaranteed
  no-re-bill: a step whose first attempt dies mid-call skips on Inngest retry
  and waits for the 15-min stale-generating self-heal (deliberate).
- Self-heal breach window: a fal call legitimately hanging >15 min can be
  flipped failed by a concurrent preview and double-generated by a dispatch in
  that window; state converges on completion (edge-of-edge).
- Log-prefix split [shot-image] (service) vs [shot/image] (route); orchestrator
  catch comment slightly overstates ("row already marked failed by the
  service" — a DB-read failure leaves it pending); graceTimeoutRef never
  nulled after fire/clear; CLI-generated dialog.tsx formatting quirk.
- GET /shots presigns every shot per poll (local HMAC, mirrors GET /entities).
- feature.md wording: function-level retries:1 never engages for per-item wave
  steps (their catch swallows) — add one clarifying sentence.
- Pre-existing (not this branch): npm run lint baseline carries 1 error in
